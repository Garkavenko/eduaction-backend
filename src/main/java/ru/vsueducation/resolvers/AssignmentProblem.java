 ...
 * @version 1.0
 */
public class AssignmentProblem implements GetJsonResult {
    ...
    public AssignmentProblem(int[][] matrix) throws IllegalAccessException {
        if (matrix.length != matrix[0].length) {
            throw new IllegalAccessException("The matrix is not square!");
        }

        this.matrix = matrix;
        this.matrixCopy = new int[matrix.length][];
        for (int i = 0; i < matrix.length; i++) {
            this.matrixCopy[i] = new int[matrix[i].length];
            System.arraycopy(matrix[i], 0, this.matrixCopy[i], 0, matrix[i].length);
        }
    ...
    public static void main(String[] args) throws IllegalAccessException {
        ...
        final AssignmentProblem assignmentProblem = new AssignmentProblem(dataMatrix);
    ...
    public String getJsonResult() {
        final int[][] assignment = this.findOptimalAssignment();
        ...
        for (int i = 0; i < assignment.length; i++) {
            total += matrixCopy[assignment[i][0]][assignment[i][1]];
